...
def process_folder_function(process_folder_name, current_directory):
  ...
  with open(process_folder_path+robotTestFileName, 'w') as test, open(process_folder_path+robotResourcesFileName, 'w') as resources:
    ...
    test.write('# Prompt Command to Execute the BlindBatch : \n')
    test.write('# robot -i TC_BlindBatch '+robotTestFileName+'\n')
    # Settings section
    test.write('*** Settings ***\n')
    test.write('Library    FakerLibrary    #locale=pt_BR \n')
    test.write('Library    OperatingSystem \n')
    test.write('Library    Collections \n')
    test.write('Library    DateTime \n')
    ...
    test.write('    kwLogin\n')
    test.write('    ${start_time}=    Get Time    epoch\n')
    test.write('    FOR    ${i}    IN RANGE    30\n')
    ...
    test.write('    END\n')
    test.write('    ${end_time}=    Get Time    epoch \n')
    test.write('    ${time_spent}=    Subtract Time From Time    ${end_time}    ${start_time}    number \n')
    ...
    test.write('kwFakerDataSetup\n')
    test.write('    [Documentation]  FakerLibrary doc: https://marketsquare.github.io/robotframework-faker/ \n')
...
# process_folder_name = 'friendlyShoulder-exclusive-all-types-no-scalation'
# process_folder_name = 'friendlyShoulder-exclusive-all-types-with-scalation'
# process_folder_name = 'friendlyShoulder-inclusive-all-types-no-scalation'
# process_folder_name = 'friendlyShoulder-inclusive-all-types-with-scalation'
# process_folder_name = 'friendlyShoulder-only-complaint-no-scalation'
# process_folder_name = 'friendlyShoulder-only-complaint-with-scalation'
# process_folder_name = 'friendlyShoulder-only-suggestion-compliment'
# process_folder_name = 'friendlyShoulder-parallel-all-types-no-scalation'
# process_folder_name = 'friendlyShoulder-parallel-all-types-with-scalation'
# process_folder_name = 'friendlyShoulder-parallel-all-types-with-scalation'
# 12 Travel Plan
...
# print(f'Time spent processing {process_folder_name}: {time_spent} seconds')
